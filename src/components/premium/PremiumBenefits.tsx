...
import { premiumProfileFor, diagnosticAreas, PREMIUM_BONUS_COUNT } from "@/data/premiumBonus";
import { premiumVolume, formatKRW } from "@/data/pricing";

// 200P+ 대상 상품 상세페이지에만 노출되는 Premium Benefits 영역.
// 혜택 7종의 제목·설명은 해당 교재의 시험·과목에 맞춰 자동으로 바뀝니다.
// (노출 조건은 호출부에서 premiumEligible() 로 판단합니다.)

export default function PremiumBenefits({ product }: { product: Product }) {
  const profile = premiumProfileFor(product);
  const areas = diagnosticAreas(product);
  ...
  return (
    <section className="mt-10 border border-brass-500/35 bg-navy-950 p-6 text-ivory-100 lg:p-8">
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 border-b border-ivory-100/15 pb-4">
        <div>
          <p className="font-label text-[10.5px] uppercase tracking-[0.18em] text-brass-500">{profile.label}</p>
      ...
      {profile.tagline && (
        <p className="mt-4 font-display text-[18px] font-semibold text-brass-500">{profile.tagline}</p>
      )}
      {profile.copy && (
        <p className={`${profile.tagline ? "mt-2" : "mt-4"} text-[13.5px] leading-relaxed text-ivory-100/75`}>
          {profile.copy}
        </p>
      )}
      ...
      <div className="mt-5 grid gap-px bg-ivory-100/12 sm:grid-cols-2">
        {profile.bonuses.map((b) => (
          <div key={b.key} className="flex items-start justify-between gap-3 bg-navy-950 p-4">
            <div className="flex items-start gap-3">
              <span className="font-display text-[15px] font-semibold leading-none text-brass-500/70">{b.no}</span>
              <div>
                <p className="font-label text-[11.5px] uppercase leading-tight tracking-[0.09em] text-ivory-100">
                  {b.en}
                </p>
                <p className="mt-1 text-[12px] leading-snug text-ivory-100/60">{b.short}</p>
                {b.key === "weaknessCheck" && areas.length > 0 && (
                  <p className="mt-1.5 font-label text-[9px] uppercase tracking-[0.08em] text-brass-500/70">
                    {areas.join(" · ")}
                  </p>
                )}
              </div>
      ...
      <p className="mt-5 text-[12px] leading-relaxed text-ivory-100/50">
        Premium Bonus Materials {PREMIUM_BONUS_COUNT}종은 200페이지 이상 구성을 선택하신 경우에 제공되며,
        구매 확인 후 본교재와 함께 디지털 자료로 안내해 드립니다.
      </p>
    </section>
  );
}

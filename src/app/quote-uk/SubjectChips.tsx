...
function buildIntro(year: Year, subject: string, toc: string[]): string {
  ...
  const isExam = year === "11+" || year === "ISEB" || year === "UKiset";
  const isAlevel = year === "AS" || year === "A2";
  if (isExam) {
    return `이 구성은 ${year} ${subject} 대비 문제집입니다. ${sample} 등을 연습합니다. 학교·학원 커리큘럼과 다를 수 있으며, 필요한 범위는 카카오톡으로 알려주세요.`;
  }
  if (isAlevel) {
    return `이 구성은 ${year} ${subject} 대비 문제집입니다. ${sample} 등 A-level 범위를 다룹니다. 시험국(AQA, Edexcel, OCR)을 카카오톡에 알려주시면 문항 톤을 맞춥니다.`;
  }
  return `이 구성은 ${year} ${subject} 문제집입니다. ${sample} 등을 중심으로 연습합니다. 영국 National Curriculum / GCSE 범위에 맞춰 구성하며, 시험국을 알려주시면 문항 톤을 맞춥니다.`;
}
